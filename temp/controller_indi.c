...
static vector_t positionDesired;		//ev_tag 
static vector_t velocityDesired;		//ev_tag 
		...
		const uint32_t tick)
{
	...
	if (RATE_DO_EXECUTE(POSITION_RATE, tick)) {
		//positionController(&actuatorThrust, &attitudeDesired, setpoint, state);		//ev_tag

		// Get reference position 						//ev_tag
		positionDesired.x = setpoint->position.x;		//ev_tag
		positionDesired.y = setpoint->position.y;		//ev_tag
		positionDesired.z = setpoint->position.z;		//ev_tag

		// P-Term																//ev_tag
		// TODO: define x_kp, y_kp, z_kp										//ev_tag
		velocityDesired.x = x_kp * (positionDesired.x - state->position.x);		//ev_tag
		velocityDesired.y = y_kp * (positionDesired.y - state->position.y);		//ev_tag
		velocityDesired.z = z_kp * (positionDesired.z - state->position.z);		//ev_tag
